...
public class JSONWriter : ImmortalObject<JSONWriter>
{
    ...
    { 
        ...
        List<int> characterStatusCoefficient = new List<int>
        {
            ...
            friendlyOperator.Dna.TotalSpeedCoefficient
        };
    ...
    private FriendlyOperator ReadFriendlyOperatorData(OperatorBattleStatus operatorBattleStatus)
    {
        ...
        //string filepath = Path.Combine(CustomPath, $"{friendlyOperator.TrustData.CharacterName}.json");//1경로에 2이름으로 저장한다? 틀렸다
        string characterName = friendlyOperator.TrustData.CharacterName;
        string folderPath = Path.Combine(Application.persistentDataPath, "Json");
        string filePath = Path.Combine(folderPath, $"{characterName}.json");
        if (File.Exists(filePath))
        {
            ...
            /*Debug.Log($"Name: {charactersData.CharacterName},\n" +
                      $"GeneIDsCount: {charactersData.GeneIDList.Count}" + 
                      $"Trust: {charactersData.Trust}");*/
            ...
            if (friendlyOperator.Dna.CycleSkill == null)
            {
                Debug.LogError("CycleSkill is null!");
            }*/
            //Dna가 널이라고?
    ...
    public void SaveGeneData(Gene gene)
    {
        ...
        //정보추출.. 할것도없네 이렇게하면?
        GeneData data = new GeneData(gene);
        ...
        //파일 경로 설정
        string filePath = Path.Combine(folderPath, $"{data.GeneID}{(GeneType)data.GeneType}.json");
    ...
    public List<Gene> ReadGeneData()
    {
        ...
        for (int i = 0; i < jsonFiles.Length; i++)
        {
            try
            {
                ...
                GeneData data = JsonUtility.FromJson<GeneData>(jsonContent);

                if (data != null)
                {
                    geneDataList.Add(data);
    ...
    public void SaveRenewalGeneData(Gene gene)
    {
        ...
        for (int i = 0; i < jsonFiles.Length; i++)
        {
            ...
            if (data != null)
            {
                if (gene.GeneID == data.GeneID)
                {
                    GeneData targetData = new GeneData(gene);
                    //파일 내부 형식? 설정
                    string targetJsonContent = JsonUtility.ToJson(targetData, true); // true = 보기 좋은 포맷(들여쓰기)
