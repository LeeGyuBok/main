...
public class MySecondAStar : MortalManager<MySecondAStar>
{
    ...
    private GameObject selectedMapObject;
    ...
    private List<Vector3> startPositionDifferenceBetweenOperators;
    ...
    void Start()//
    {
        ...
        startPositionDifferenceBetweenOperators = new List<Vector3>()
        {
            new Vector3(0.125f, 0.6f, 0f),
            new Vector3(-0.125f, 0.6f, 0f),
            new Vector3(0f, 0.6f, 0f),
            new Vector3(0f, 0.6f, 0.125f),
            new Vector3(0f, 0.6f, -0.125f)
        };

        selectedMapObject = GameImmortalManager.Instance.campaignMapObject;
        friendlyOperators = GameImmortalManager.Instance.selectedOperators;
    }
    ...
    public void Spawn()
    {
        ...
        {
            Vector3 spawn = startPositionDifferenceBetweenOperators[i];
            
            Instantiate(friendlyOperators[i], StartNode.WorldPosition + spawn, quaternion.identity);    
        }
    ...
    private void Initialize()
    {
        ...
        GameObject selectedMap = Instantiate(selectedMapObject, Vector3.zero, Quaternion.identity);
        ...
        // Plane 크기 계산. 정사각형 또는 직사각형이어야할듯? < 골든 정답
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int z = 0; z < gridSizeZ; z++)
            {
                ...
                //오후 3:45 2024-12-20 현재 선택된 버튼에 의해 맵을 할당받고있음. 근데왜 여기 false임?
                ...
                if (isStart)
                {
                    ...
                    Debug.Log($"startNodes {Grid[x, z].WorldPosition}");
                    //여기는 정상.
                ...
                if (isTarget)
                {
                    ...
                    Debug.Log($"targetNodes {Grid[x, z].WorldPosition}");
                ...
                if (isReturn)
                {
                    ...
                    Debug.Log($"returnNodes {Grid[x, z].WorldPosition}");
    ...
    private List<MyNode> FindPath(MyNode start, MyNode target)
    {
        ...
        while (possibleNodes.Count > 0)
        {
            ...
            Instantiate(testGizmo, current.WorldPosition, Quaternion.identity);
